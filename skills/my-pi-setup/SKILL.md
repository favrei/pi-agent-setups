...
| `claude_code_version_too_old` despite installed Claude Code ≥ 2.1.280 | `pi-background-tasks` 2.6.2/2.6.3 independently reports 2.1.251. Copy `patches/pi-background-tasks-2.6.2-2.6.3-claude-code-2.1.280.py` (do not run it as part of installation); tell the user how to run it and `/reload`. It patches both version and User-Agent and must be reapplied after `pi update --extensions`. |
