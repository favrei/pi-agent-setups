...
| `pi_anthropic_attribution_transcript_unsupported: system messages require gateway-injected Pi transcript helpers` | The `@earendil-works/pi-ai` copy in `~/.pi/agent/npm/node_modules/` is older than the running pi (< 0.86 lacks the transcript helpers). Tell the user to run `npm install --prefix ~/.pi/agent/npm --save-exact --legacy-peer-deps --ignore-scripts "@earendil-works/pi-ai@$(pi --version)"`, then `/reload`. Do not run it as part of installation. Repeat after upgrading pi. |
| Anthropic errors on `pi-background-tasks` < 2.6.5 (transcript hang at 100% CPU, no Opus 5.5 policy, `claude_code_version_too_old`) | Fixed upstream by 2.6.5. Run `pi update --extensions`, then check the SDK copy above. The old local patch scripts are retired; delete leftover `~/.pi/agent/patches/pi-background-tasks-*.py`. |
