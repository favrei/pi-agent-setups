...
The manager (main session, stronger model) delegates with a message such as: "Use the worker subagent. Brief: `.agents/memory/inbox/brief-03-train-kd.md`." Each delegation starts with a clean context. If your version supports resuming a subagent, use that for redirects; otherwise re-brief.
...
```bash
claude -p "$(cat path/to/manager-loop/references/worker-prompt.md .agents/memory/inbox/brief-03-train-kd.md)" \
  --model sonnet --output-format json > .agents/memory/inbox/run-03.json
# redirect: resume that session with the delta
claude -p "$(cat .agents/memory/inbox/brief-03-redirect-1.md)" --resume <session_id> --model sonnet
```
